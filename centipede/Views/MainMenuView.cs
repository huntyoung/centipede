...
namespace centipede
{
    public class MainMenuView : GameStateView
    {
        ...
        private int m_menuStateCount = 5;
        private int m_menuStateOn = 1;
        ...
        {
            m_fontMenu = contentManager.Load<SpriteFont>("Fonts/font1");
            m_fontMenuSelect = contentManager.Load<SpriteFont>("Fonts/font2");
        }
        public override GameStateEnum processInput(GameTime gameTime)
        {
            ...
            if (!m_waitForKeyRelease)
            {
                ...
                if (Keyboard.GetState().IsKeyDown(Keys.Down) && m_menuStateOn < m_menuStateCount)
                {
                    m_menuStateOn++;
                    m_currentSelection = m_currentSelection + 1;
                    m_waitForKeyRelease = true;
                }
                if (Keyboard.GetState().IsKeyDown(Keys.Up) && m_menuStateOn > 1)
                {
                    m_menuStateOn--;
                    m_currentSelection = m_currentSelection - 1;
                    m_waitForKeyRelease = true;
